...
import { Record } from "./uesListeType";
...
export default function useListe() {
	...
	const renderListDetail = () => {
		...
		const renderDetailsInSpan = processing.map((array, index) => (
			<span key={index}>{array.word}</span>
		));
	...
	const addRecord = ({ input, answer, result }: Record) => {
		setRecord([
			...record,
			{ input: input, answer: answer, result: result },
		]);
	};
	...
	const onClickNextButton = () => {
		...
		const clickedBTNtextContent = input.current;
		addRecord({
	...
	const RenderResult = () => {
		return (
			<div>
				{record.map((result, index) => {
					return (
						<li
							key={index}
							className={
								result.result ? "bg-lime-500" : "bg-red-500"
							}
						>
							...
							{!result.result && (
								<span className="font-bold bg-lime-500">
									{" "}
									{result.answer}
								</span>
							)}
				...
				<span>
					Ton score final est de {countTrueRecord()} sur{" "}
					{record.length}
				</span>
			</div>
		);
